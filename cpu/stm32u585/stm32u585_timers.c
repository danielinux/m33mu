...
void mm_stm32u585_timers_tick(mm_u64 cycles)
{
    size_t i;
    for (i = 0; i < sizeof(timers) / sizeof(timers[0]); ++i) {
        tim_tick(&timers[i], cycles);
    }
    mm_stm32u585_watchdog_tick(cycles);
}

void mm_stm32u585_timers_init(struct mmio_bus *bus, struct mm_nvic *nvic)
{
    static const mm_u32 bases[] = {
        0x40000000u, 0x40000400u, 0x40000800u, 0x40000C00u
    };
    static const int irq_map[] = { 45, 46, 47, 48 };
    mm_u32 *tz = mm_stm32u585_tzsc_regs();
    mm_u32 *tz1 = tz != 0 ? tz + (0x10u / 4u) : 0;
    size_t i;
    g_nvic = nvic;
    mm_stm32u585_exti_set_nvic(nvic);
    for (i = 0; i < sizeof(timers) / sizeof(timers[0]); ++i) {
        ...
        t->psc = 0u;
        t->psc_accum = 0u;
        t->rcc_regs = mm_stm32u585_rcc_regs();
        t->sec_reg = tz1;
        t->sec_bitmask = (1u << i);
...
void mm_stm32u585_timers_reset(void)
{
    size_t i;
    for (i = 0; i < sizeof(timers) / sizeof(timers[0]); ++i) {
