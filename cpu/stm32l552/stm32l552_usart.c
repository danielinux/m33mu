...
static mm_bool clock_apb2_usart1(struct usart_inst *u)
{
    ...
    return ((u->rcc_regs[0x60 / 4] >> 14) & 1u) != 0u;
}
...
static mm_bool clock_apb1enr1_generic(struct usart_inst *u)
{
    ...
    if (u->rcc_regs == 0) return MM_TRUE;
    return ((u->rcc_regs[0x58 / 4] >> bit) & 1u) != 0u;
}

static mm_bool clock_apb3_lpuart1(struct usart_inst *u)
{
    if (u->rcc_regs == 0) return MM_TRUE;
    return ((u->rcc_regs[0x5c / 4] >> 0) & 1u) != 0u;
}
...
void mm_stm32l552_usart_poll(void)
{
    size_t i;
...
void mm_stm32l552_usart_init(struct mmio_bus *bus, struct mm_nvic *nvic)
{
    static const mm_u32 bases[] = {
        0x40013800u, 0x40004400u, 0x40004800u, 0x40004C00u,
        0x40005000u, 0x40008000u
    };
    ...
    size_t i;
    mm_u32 *tz1 = mm_stm32l552_tzsc_regs();
    ...
    for (i = 0; i < usart_count; ++i) {
        ...
        u->rcc_regs = mm_stm32l552_rcc_regs();
        u->clock_on = 0;
        ...
        if (i == 0) {
            ...
            u->sec_bitmask = (1u << 1); /* TZSC_SECCFGR2 USART1SEC bit1 */
            u->sec_reg = tz1_cfgr2;
        } else if (i >= 1 && i <= 4) {
            u->clock_on = clock_apb1enr1_generic;
            u->sec_bitmask = (1u << (i + 9u)); /* USART2..UART5 bits 10..13 */
        } else {
            u->clock_on = clock_apb3_lpuart1;
            u->sec_reg = tz1_cfgr1;
            u->sec_bitmask = (1u << 21); /* TZSC_SECCFGR1 LPUART1SEC bit21 */
        }
        reg.base = bases[i];
        reg.size = 0x400u;
        reg.opaque = u;
...
void mm_stm32l552_usart_reset(void)
{
    size_t i;
