...
static mm_bool stm32h563_gpio_bank_info(void *opaque, int bank, char *name_out, size_t name_len, int *pins_out)
{
    (void)opaque;
    if (bank < 0 || bank >= 9) {
        return MM_FALSE;
...
void mm_stm32h563_mmio_reset(void)
{
    ...
    for (i = 0; i < sizeof(gpio) / sizeof(gpio[0]); ++i) {
        ...
        gpio_ctx_data[i * 2].clock_enabled = stm32h563_gpio_clock_enabled_cb;
        ...
        gpio_ctx_data[i * 2 + 1].clock_enabled = stm32h563_gpio_clock_enabled_cb;
...
static mm_u32 flash_sector_size(void)
{
     ...
     * sectors are 8 Kbytes on STM32H5 (RM0481, 128 sectors per bank), so
