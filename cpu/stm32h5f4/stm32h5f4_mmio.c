...
#define MPCBB4_BASE     0x40033800u
#define MPCBB4_SEC_BASE 0x50033800u
#define MPCBB5_BASE     0x40033c00u
#define MPCBB5_SEC_BASE 0x50033c00u
...
#define FLASH_SECTOR_COUNT 512u
...
#define FLASH_SECWM_STRT_MASK  0xFFu
#define FLASH_SECWM_END_SHIFT  16
#define FLASH_SECWM_EMPTY  0x000000FFu /* STRT=0xFF, END=0: no secure sector */
...
static struct mpcbb_state mpcbb[5];
...
static struct stm32_gpio_state gpio[11]; /* A..K */
static struct stm32_gpdma_state gpdma1;
static struct stm32_gpdma_state gpdma2;
static struct stm32_gpio_ctx gpio_ctx_data[22]; /* NS + S for each bank */
...
/* GPDMA channel interrupts are not contiguous on the STM32H5F4: channels 0..7
 * sit right after each controller's base vector, channels 8..11 are grouped
 * together much higher up. */
static const mm_u16 gpdma1_irq_map[12] = {
    27u, 28u, 29u, 30u, 31u, 32u, 33u, 34u, 136u, 137u, 138u, 139u
};
static const mm_u16 gpdma2_irq_map[12] = {
    90u, 91u, 92u, 93u, 94u, 95u, 96u, 97u, 140u, 141u, 142u, 143u
};
/* SECCFGR words per bank: one word per 32 blocks of 512 bytes, i.e. one word
 * per SRAM super-block (RM0517 Table 6: SRAM1..5 have 16/8/24/24/24). */
static const mm_u32 mpcbb_words[] = { 16u, 8u, 24u, 24u, 24u };

static mm_u32 stm32h5f4_gpio_bank_read(void *opaque, int bank);
static mm_u32 stm32h5f4_gpio_bank_read_moder(void *opaque, int bank);
static mm_bool stm32h5f4_gpio_bank_clock(void *opaque, int bank);
static mm_u32 stm32h5f4_gpio_bank_read_seccfgr(void *opaque, int bank);
static void exti_gpio_update(int bank, mm_u32 old_level, mm_u32 new_level);
static mm_bool stm32h5f4_gpio_clock_enabled_cb(int bank);
...
static mm_bool stm32h5f4_rcc_clock_list_line(void *opaque, int line, char *out, size_t out_len);

static mm_bool stm32h5f4_gpio_bank_info(void *opaque, int bank, char *name_out, size_t name_len, int *pins_out)
{
    (void)opaque;
    if (bank < 0 || bank >= 11) {
        return MM_FALSE;
...
void mm_stm32h5f4_mmio_reset(void)
{
    ...
    gpdma1.instance = 0;
    gpdma1.num_channels = 12; /* STM32H5F4 GPDMA has 12 channels per instance */
    gpdma1.irq_base = 27; /* GPDMA1 channel IRQs start at 27 */
    gpdma1.irq_map = gpdma1_irq_map;
    stm32_gpdma_reset(&gpdma1);
    gpdma2.instance = 1;
    gpdma2.num_channels = 12;
    gpdma2.irq_base = 90; /* GPDMA2 channel IRQs start at 90 */
    gpdma2.irq_map = gpdma2_irq_map;
    ...
    for (i = 0; i < sizeof(gpio) / sizeof(gpio[0]); ++i) {
        ...
        gpio_ctx_data[i * 2].clock_enabled = stm32h5f4_gpio_clock_enabled_cb;
    ...
    mm_gpio_bank_set_seccfgr_reader(stm32h5f4_gpio_bank_read_seccfgr, 0);
    mm_gpio_set_bank_info_reader(stm32h5f4_gpio_bank_info, 0);
    mm_rcc_set_clock_list_reader(stm32h5f4_rcc_clock_list_line, 0);
    ...
    /* RNG reset values */
    rng.regs[RNG_CR_OFFSET / 4] = 0x00200f00u;
...
void mm_stm32h5f4_watchdog_tick(mm_u64 cycles)
{
    ...
    mm_u64 cpu_hz = mm_stm32h5f4_cpu_hz();
...
mm_bool mm_stm32h5f4_register_mmio(struct mmio_bus *bus)
{
    ...
    /* MPCBB1..5 (non-secure and secure aliases) */
    reg.size = MPCBB_SIZE;
    reg.read = mpcbb_read;
    ...
    reg.base = MPCBB5_BASE;
    reg.opaque = &mpcbb[4];
    if (!mmio_bus_register_region(bus, &reg)) return MM_FALSE;
    reg.base = MPCBB5_SEC_BASE;
    if (!mmio_bus_register_region(bus, &reg)) return MM_FALSE;
