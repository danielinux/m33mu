...
static int test_adc_imm_carry(void)
{
    static const mm_u8 code[] = {
        0x62, 0x1e,             /* subs r2, r4, #1 */
        0x45, 0xf1, 0xff, 0x33  /* adc.w r3, r5, #0xffffffff */
    };
    struct mm_memmap map;
    struct mm_scs scs;
    ...
    if (run_sequence(code, sizeof(code), &cpu, &map, &scs, &gdb) != 0) {
        printf("exec_sp_clamp_ct_test: execution failed (carry)\n");
        return 1;
    }
    if (cpu.r[3] != 0x12345678u) {
        printf("exec_sp_clamp_ct_test: r3 mismatch (carry) got=0x%08lx\n", (unsigned long)cpu.r[3]);
        return 1;
    }
    return 0;
}

static int test_adc_imm_no_carry(void)
{
    static const mm_u8 code[] = {
        0x62, 0x1e,             /* subs r2, r4, #1 */
        0x45, 0xf1, 0xff, 0x33  /* adc.w r3, r5, #0xffffffff */
    ...
    if (run_sequence(code, sizeof(code), &cpu, &map, &scs, &gdb) != 0) {
        printf("exec_sp_clamp_ct_test: execution failed (no carry)\n");
        return 1;
    }
    if (cpu.r[3] != 0x12345677u) {
        printf("exec_sp_clamp_ct_test: r3 mismatch (no carry) got=0x%08lx\n", (unsigned long)cpu.r[3]);
        return 1;
    }
    return 0;
}
...
int main(void)
{
    ...
    if (test_adc_imm_carry() != 0) {
        printf("FAIL: adc_imm_carry\n");
        failures++;
    } else {
        printf("PASS: adc_imm_carry\n");
    }
    if (test_adc_imm_no_carry() != 0) {
        printf("FAIL: adc_imm_no_carry\n");
        failures++;
    } else {
        printf("PASS: adc_imm_no_carry\n");
    }
    if (failures != 0) {
        return 1;
    }
