/* gpio-input-exti-falling: FTSR1 only (RTSR1 left clear). The trace has
 * 2 rising and 2 falling crossings; only the falling ones should produce
 * an interrupt. Validates exti_gpio_update() correctly gates on FTSR1
 * alone, ignoring rising transitions when RTSR1 isn't armed.
 *
...
void Reset_Handler(void)
{
    ...
    EXTI_FTSR1 |= (1u << PIN); /* falling only */
    EXTI_IMR1  |= (1u << PIN);
